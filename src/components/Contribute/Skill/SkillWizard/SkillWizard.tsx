...
const DefaultSkillFormData: SkillFormData = {
  ...
  documentOutline: '',
  filePath: '',
  seedExamples: createDefaultSeedExamples(true),
  titleWork: '',
  licenseWork: '',
  creators: ''
};
...
export const SkillWizard: React.FunctionComponent<Props> = ({ skillEditFormData, isGithubMode }) => {
    ...
    () => [
      ...
      },
      {
        ...
        component: (
          <FilePathInformation
            rootPath="skills"
            path={skillFormData.filePath}
            setFilePath={(filePath) => setSkillFormData((prev) => ({ ...prev, filePath }))}
          />
        ),
      ...
      },
      {
        ...
        status: isSeedExamplesValid(skillFormData) ? StepStatus.Success : StepStatus.Error
      },
      ...(isGithubMode
        ? [
            {
        ...
        : []),
      {
        ...
        component: <ReviewSubmission contributionFormData={skillFormData} isSkillContribution isGithubMode={isGithubMode} />,
        status: StepStatus.Default
      }
    ],
    [isGithubMode, skillEditFormData?.isEditForm, skillFormData]
  );
  ...
  const handleSubmit = async (githubUsername: string): Promise<boolean> => {
    ...
    if (result) {
      setSkillFormData(DefaultSkillFormData);
    }
    return result;
  };
  ...
  return (
    <>
      <ContributionWizard
        ...
        setFormData={(formData: ContributionFormData) => setSkillFormData(formData as SkillFormData)}
